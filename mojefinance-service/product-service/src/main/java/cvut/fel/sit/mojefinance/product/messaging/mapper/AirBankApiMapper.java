...
public interface AirBankApiMapper extends TransactionsApiMapper {
    @Mapping(target = "products", source = "accounts")
    ProductsResponse toProductsResponse(AccountList accountList, @Context BankDetails bankDetails);
    ...
    Amount toDomainBalance(BalanceList balanceList);

    @Named("getAmountValue")
    default BigDecimal getAmountValue(List<BalanceListBalancesInner> balances) {
        BalanceListBalancesInner selectedBalance = balances.stream()
    ...
    @Named("getCurrency")
    default String getCurrency(List<BalanceListBalancesInner> balances) {
        BalanceListBalancesInner selectedBalance = balances.stream()
                .filter(b -> CLAV_TYPE_CODE.equals(b.getType().getCodeOrProprietary().getCode().name()))
                .findFirst()
                .orElse(balances.get(0));
