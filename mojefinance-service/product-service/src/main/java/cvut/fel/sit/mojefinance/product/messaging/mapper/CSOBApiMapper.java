...
public interface CSOBApiMapper extends TransactionsApiMapper {
    @Mapping(target = "products", source = "accounts")
    ProductsResponse toProductsResponse(GetAccountsRes getAccountsRes, @Context BankDetails bankDetails);
    ...
    Amount toDomainBalance(GetAccountBalanceRes getAccountBalanceRes);

    @Named("getAmountValue")
    default BigDecimal getAmountValue(List<BalanceInfo> balances) {
        BalanceInfo selectedBalance = balances.stream()
    ...
    @Named("getCurrency")
    default String getCurrency(List<BalanceInfo> balances) {
        BalanceInfo selectedBalance = balances.stream()
                .filter(b -> CLAV_TYPE_CODE.equals(b.getType().getCodeOrProprietary().getCode().name()))
                .findFirst()
                .orElse(balances.get(0));
