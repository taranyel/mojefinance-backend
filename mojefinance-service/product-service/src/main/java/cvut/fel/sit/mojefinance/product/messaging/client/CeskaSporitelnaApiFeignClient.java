...
import cvut.fel.sit.cs.openapi.model.MyAccountsIdTransactionsGet200Response;import cvut.fel.sit.mojefinance.product.messaging.config.CeskaSporitelnaFeignConfig;
...
public interface CeskaSporitelnaApiFeignClient {
    ...
    @GetMapping(value = "${external.api.ceska-sporitelna.transactions-path}",
            consumes = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<MyAccountsIdTransactionsGet200Response> getTransactions(
            @RequestHeader("Authorization") String authorization,
            @RequestHeader("WEB-API-key") String apiKey,
            @PathVariable String id,
